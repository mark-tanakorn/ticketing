              ...
              <div>
                ...
                <div className="border-t pt-6">
                  ...
                  {selectedTicket.status === 'approval_denied' ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {(selectedTicket.status === 'open' || selectedTicket.status === 'in_progress' || selectedTicket.status === 'sla_breached' || selectedTicket.status === 'closed') && (
                        <>
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Approved At</label>
                            <div className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50 text-gray-700">
                              {selectedTicket.approver_decided_at ? (() => {
                                const date = new Date(selectedTicket.approver_decided_at);
                                return `${date.toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: '2-digit' })} ${date.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', hour12: false })}`;
                              })() : 'N/A'}
                            </div>
                          </div>
                          ...
                          <div>
                            ...
                            <div className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50 text-gray-700">
                              {selectedTicket.severity === 'critical' ? '4' :
                               selectedTicket.severity === 'high' ? '24' :
                               selectedTicket.severity === 'medium' ? '48' : '72'}
                            </div>
                          </div>
                          ...
                          <div>
                            ...
                            <div className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50 text-gray-700">
                              {selectedTicket.sla_breached_at ? (() => {
                                const date = new Date(selectedTicket.sla_breached_at);
                                return `${date.toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: '2-digit' })} ${date.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', hour12: false })}`;
                              })() : 'N/A'}
                            </div>
                          </div>
                      ...
                      <div className="md:col-span-3">
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                          {selectedTicket.status === 'closed' ? 'Status' : 'Time Left Until SLA Breach'}
                        </label>
