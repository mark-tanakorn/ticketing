...
class TestResolveVariable:
    ...
    def test_resolve_trigger_pseudo_node(self):
        """Test resolving trigger payload via reserved pseudo-node {{trigger.*}}"""
        variables = {
            "_nodes": {
                "node1": {"phone": "+1234"}  # Ensure normal nodes still work
            },
            "trigger_data": {
                "title": "Printer broken",
                "severity": "high",
                "approver": {"phone": "+6599990000"},
                "items": [{"id": 1}, {"id": 2}],
            }
        }
        
        assert resolve_variable("trigger.title", variables) == "Printer broken"
        assert resolve_variable("trigger.approver.phone", variables) == "+6599990000"
        assert resolve_variable("trigger.items.1.id", variables) == 2
    
    def test_resolve_trigger_pseudo_node_missing(self):
        """Test resolving trigger pseudo-node returns None if trigger_data is absent"""
        variables = {"_nodes": {}}
        assert resolve_variable("trigger.title", variables) is None
    
    def test_resolve_system_pseudo_node(self):
        """Test resolving legacy system pseudo-node {{system.current_date}}"""
        # We only validate that it returns a YYYY-MM-DD formatted string.
        variables = {}
        value = resolve_variable("system.current_date", variables)
...
class TestResolveTemplate:
    ...
    def test_resolve_template_system_variable(self):
        ...
        # Should have date appended (YYYY-MM-DD)
        assert len(result) >= len("Current date: ") + 10
