...
def resolve_variable(variable_path: str, variables: Dict[str, Any]) -> Optional[Any]:
    ...
    # ------------------------------------------------------------------
    # Reserved pseudo-nodes (global execution context)
    # ------------------------------------------------------------------
    # These are NOT normal workflow nodes, but convenience namespaces that
    # can be referenced from anywhere templates are supported.
    #
    # 1) {{trigger.*}}  -> execution trigger payload (trigger_data)
    # 2) {{system.*}}   -> system variables (backward compatible; preferred UX is {current_date})
    if node_key == "trigger":
        node_data = variables.get("trigger_data")
        if node_data is None:
            logger.debug("Pseudo-node 'trigger' not available (no trigger_data)")
            return None
    elif node_key == "system":
        # Support legacy {{system.current_date}} style, even though the recommended
        # syntax for system variables is single braces: {current_date}.
        # We treat the remainder of the path as the system variable name.
        var_name = ".".join(field_path).strip()
        value = get_system_variable(var_name)
        if value is not None:
            logger.debug(f"Resolved system pseudo-node '{var_name}' → {value}")
            return value
        logger.debug(f"System pseudo-node variable '{var_name}' not found")
        return None
    else:
        # Navigate through _nodes namespace (normal node outputs)
    ...
    # Navigate through nested fields (dict keys and list indices)
    ...
    for i, field_key in enumerate(field_path):
        if isinstance(current_value, dict):
            ...
            if current_value is None:
                ...
                return None
        elif isinstance(current_value, (list, tuple)) and field_key.isdigit():
            idx = int(field_key)
            if 0 <= idx < len(current_value):
                current_value = current_value[idx]
            else:
                partial_path = ".".join([node_key] + field_path[:i+1])
                logger.debug(f"Index '{partial_path}' out of range")
                return None
        else:
