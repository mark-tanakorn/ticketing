...
#### Trigger Payload (Reserved Pseudo-Node)

When a workflow is started with `trigger_data` (e.g., via API/webhook execution), the payload is available everywhere templates work using:

```
{{trigger.title}}
{{trigger.severity}}
{{trigger.approver.phone}}
```

This is **dynamic**: you can send any JSON shape and reference whatever keys you included.
...
> Note: For backward compatibility, `{{system.current_date}}` also works, but the recommended UX is single braces like `{current_date}`.
